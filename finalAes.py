...
if __name__ == '__main__':
    ...
    while True:
        print('1es gomb titkositas, 2es gomb visszafejtes)
        mode = input()
        if mode not in ['1', '2']:
            print('vagy "1"es vagy "2"es')
            continue
        else:
            break
    while True:
        if len(key) > 16:
            print('16 bytenal nem lehett hoszabb a kulcs')
            continue
        
        for symbol in key:
            if ord(symbol) > 0xff:
                print('0-9a-Z valasz karaktereket')
                continue
    ...
    if mode == '1':
        ...
        out_path = os.path.join(os.path.dirname('.') , 'crypted_' +str(time.time()))
        ...
        out_path = os.path.join(os.path.dirname('.') , 'decrypted_' +str(time.time()))
